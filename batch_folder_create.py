...
while(1):
    ...
    if batch_mode == True:
        ...
        for i in countup(job_list_length):
            output += '\n' + str(i + 1) + '\t' + common_name[i] + '\t\t' + destination_folder[i] + '\t' + start_num_string[i] + '\t' + end_num_string[i] + '\t' + destination_folder[i] + common_name[i]
        ...
        output += " from " + start_num_string[job_pos] + " to " + end_num_string [job_pos]+ " inside Folder " + destination_folder[job_pos] + ".\n\t e.g. " + destination_folder[job_pos] + common_name[job_pos]
...
for i in countup(len(start_num)):
    ...
    if leading_zero[i] == True:
        ...
        log_tools.add_to_txt_log('Completed process for ' + destination_folder[job_pos] + ' folders ' + start_str + ' to ' + str(end_num[job_pos]), add_date = True)
    else:
        start_str = str(start_num[job_pos])
        while start_num[i] <= end_num[i]:
            make_numbered_dir(destination_folder[i] + start_num_string[i])
